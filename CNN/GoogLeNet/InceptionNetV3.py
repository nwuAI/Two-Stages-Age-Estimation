...
def train_neural_network(session, optimizer, feature_batch, label_batch):
    session.run(optimizer, feed_dict={inputs: feature_batch, y: label_batch})


def print_stats(session, feature_batch, label_batch, cost, mae):
    Loss = session.run(cost, feed_dict={inputs: feature_batch, y: label_batch})
    # Accuracy = session.run(accuracy,feed_dict={x:valid_features,y:valid_labels,keep_prob:1.0})
    MAE = session.run(mae, feed_dict={inputs: feature_batch, y: label_batch})
...
with slim.arg_scope(inception_v3_arg_scope()): # scope中包含了batch normalization默认参数，激活函数和参数初始化方式的默认值
  logits, end_points = inception_v3(inputs, is_training= True) # inception_v3中传入inputs获取里logits和end_points

  
fillogits = output(logits, 85)

fillogits = tf.identity(fillogits, name='fillogits')

save_model_path = './googlenet_image_classification'


inputs = tf.placeholder(tf.float32, [None, 224, 224, 3])
y = tf.placeholder(tf.float32,[None,85])

cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=fillogits, labels=y))
optimizer = tf.train.AdamOptimizer().minimize(cost)

mae = tf.reduce_mean(tf.abs(tf.subtract(tf.argmax(fillogits, 1), tf.argmax(y, 1))))
