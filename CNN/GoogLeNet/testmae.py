...
test_features, test_labels = pickle.load(open('preprocess_test1.p', mode='rb'))
...
def inception_v3(inputs,
                 ...
                 is_training=False, # 标志是否为训练过程，只有在训练时Batch normalization和Dropout才会启用
  ...
  with tf.variable_scope(scope, 'InceptionV3', [inputs, num_classes], # 定义参数默认值
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout], # 定义标志默认值
      ...
      with tf.variable_scope('Logits'):
        net = slim.avg_pool2d(net, [5, 5], padding='VALID',
...
abs11 = tf.abs(tf.subtract(tf.argmax(fillogits, 1), tf.argmax(y, 1)))
	
saver = tf.train.Saver()
 
sess = tf.Session() # 创建session
 
test_features1 = test_features[0:500]
test_labels1 = test_labels[0:500]
saver.restore(sess, save_model_path)
MAEs = sess.run(abs11, feed_dict={inputs: test_features, y: test_labels})
MAE = np.mean(MAEs)
